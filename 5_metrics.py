...
def save_results(args, number_, all_metirc_results):
    save_instruct = args.save_instruct.split(',')
    model_name, epoch, training_set = save_instruct[0], save_instruct[1], save_instruct[2]
    with open('outputs/metric_result.txt', 'a') as file:

        result_details = f'Model: {model_name}, epoch: {epoch}' \
...
if __name__=="__main__":
  ...
  for checkpoint in args.checkpoints:
    input_file_name = args.output_dir + '/' + "test_inference_groundtruth_%s.json"%checkpoint
